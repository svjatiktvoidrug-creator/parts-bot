...
async def send_waiting_report(context):
    import datetime
    waiting = context.bot_data.get("waiting_parts", [])
    users = context.bot_data.get("users", {})
    ...
    await context.bot.send_message(chat_id=TEAM_LEAD_ID, text=report)
...
def main():
    ...
    # Schedule report every 5 days
    app.job_queue.run_repeating(send_waiting_report, interval=5*24*60*60, first=5*24*60*60)
