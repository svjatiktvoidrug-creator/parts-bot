...
def get_main_menu(user_id=None):
    buttons = [
        ...
        [KeyboardButton("⏳ Очікую запчастин")],
    ]
    if user_id and int(user_id) == TEAM_LEAD_ID:
        buttons.append([KeyboardButton("👑 Очікування запчастин майстрів")])
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in users:
        await update.message.reply_text(f"👋 З поверненням, {users[user_id]}!", reply_markup=get_main_menu(user_id))
...
async def register_first_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"✅ Зареєстровано як *{full_name}*!", parse_mode="Markdown", reply_markup=get_main_menu(str(update.effective_user.id)))
    return ConversationHandler.END
...
async def confirm_order(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        done_keyboard = InlineKeyboardMarkup([
            ...
            [InlineKeyboardButton("⏳ Немає на складі", callback_data=f"no_stock_{user_id}")]
        ])
...
async def history_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("✅ Запит відправлено!", reply_markup=get_main_menu(str(update.effective_user.id)))
    except Exception as e:
...
async def handle_group_reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    unbind_workers = context.bot_data.get("unbind_workers", {})
    reply_msg_id = str(message.reply_to_message.message_id)
    if reply_msg_id in unbind_workers:
        worker_id = unbind_workers.pop(reply_msg_id)
        context.bot_data["unbind_workers"] = unbind_workers
        try:
            await context.bot.send_message(
                chat_id=worker_id,
                text=f"Запчастину відвязано, вкажіть на стікері номер переміщення ({reply_text})"
            )
        except Exception as e:
...
async def unbind_done(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Store mapping of message_id -> worker_id for later reply handling
    if "unbind_workers" not in context.bot_data:
        context.bot_data["unbind_workers"] = {}
    sent_msg = await context.bot.send_message(
        chat_id=query.message.chat.id,
        message_thread_id=query.message.message_thread_id,
        text="📦 Для завершення відвязки — зробіть Відповідь на це повідомлення з номером переміщення"
    )
    context.bot_data["unbind_workers"][str(sent_msg.message_id)] = user_id


async def menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("Головне меню:", reply_markup=get_main_menu(str(update.effective_user.id)))


async def no_stock(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    user_id = int(query.data.replace("no_stock_", ""))

    # Get order info from caption
    caption = query.message.caption or ""
    
    # Update button - mark order as done + no stock
    await query.edit_message_reply_markup(
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("👌 Вже зроблено", callback_data="already_done")],
            [InlineKeyboardButton("⌛️ Немає на складі — очікує", callback_data="no_stock_marked")]
        ])
    )

    # Save to waiting list
    if "waiting_parts" not in context.bot_data:
        context.bot_data["waiting_parts"] = []
    
    import datetime
    msg_link = f"https://t.me/c/{str(GROUP_CHAT_ID).replace('-100', '')}/{query.message.message_id}"
    
    # Parse model and parts from caption
    order_info = ""
    for line in caption.split("\n"):
        if "Модель:" in line or "Запчастини:" in line or line.startswith("  •"):
            order_info += line.strip() + "\n"
    if not order_info:
        order_info = caption[:200]

    context.bot_data["waiting_parts"].append({
        "user_id": user_id,
        "caption": caption,
        "order_info": order_info.strip(),
        "date": datetime.datetime.now().strftime("%d.%m.%Y"),
        "msg_link": msg_link,
        "entry_id": f"{user_id}_{query.message.message_id}"
    })
    ...
    try:
        await context.bot.send_message(
            chat_id=user_id,
            text="⏳ Запчастини немає на складі. Ваше замовлення очікує надходження."
        )
    except Exception as e:
        logger.error(f"Error notifying user about no stock: {e}")


async def waiting_parts_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = str(update.effective_user.id)
    waiting = context.bot_data.get("waiting_parts", [])
    user_waiting = [w for w in waiting if str(w["user_id"]) == user_id]

    if not user_waiting:
        await update.message.reply_text("У вас немає замовлень в очікуванні.", reply_markup=get_main_menu(str(update.effective_user.id)))
        return

    for item in user_waiting:
        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton("✅ Отримав запчастину", callback_data=f"received_{item['entry_id']}")]
        ])
        date_str = item['date']
        order_info = item.get('order_info', 'Інформація відсутня')
        text = "⏳ Очікує запчастина\nДата: " + date_str + "\n\n" + order_info
        await update.message.reply_text(text, reply_markup=keyboard)


async def received_part(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    entry_id = query.data.replace("received_", "")

    waiting = context.bot_data.get("waiting_parts", [])
    context.bot_data["waiting_parts"] = [w for w in waiting if w["entry_id"] != entry_id]

    await query.edit_message_text("✅ Запчастину отримано! Замовлення закрито.")


async def send_waiting_report(context, bot=None):
    import datetime
    waiting = context.bot_data.get("waiting_parts", [])
    users = context.bot_data.get("users", {})

    if not waiting:
        _bot = bot or context.bot
    await _bot.send_message(
        chat_id=TEAM_LEAD_ID,
        text="Звіт по очікуванню запчастин: немає активних очікувань."
    )
    return
    ...
    for item in waiting:
        ...
        if name not in by_user:
            by_user[name] = []
        by_user[name].append(item)

    report = f"📊 Звіт по очікуванню запчастин ({datetime.datetime.now().strftime('%d.%m.%Y')}):\n\n"
    for name, items in by_user.items():
        report += f"👤 {name}:\n"
        for item in items:
            report += f"  • Замовлення від {item['date']}: {item['msg_link']}\n"
        report += "\n"

    _bot = bot or context.bot
    await _bot.send_message(chat_id=TEAM_LEAD_ID, text=report)


async def report_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id != TEAM_LEAD_ID:
        return
    await send_waiting_report(context)
...
async def admin_waiting_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not waiting:
        await update.message.reply_text("Немає активних очікувань запчастин.", reply_markup=get_main_menu(str(update.effective_user.id)))
        return

    # Get unique masters with waiting parts
    masters = {}
    for item in waiting:
        uid = str(item["user_id"])
        name = users.get(uid, f"ID:{uid}")
    ...
    keyboard = [[InlineKeyboardButton(name, callback_data=f"admin_master_{uid}")] for uid, name in masters.items()]
    await update.message.reply_text("Оберіть майстра:", reply_markup=InlineKeyboardMarkup(keyboard))


async def admin_master_parts(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    if update.effective_user.id != TEAM_LEAD_ID:
        return

    master_uid = query.data.replace("admin_master_", "")
    waiting = context.bot_data.get("waiting_parts", [])
    users = context.bot_data.get("users", {})
    master_name = users.get(master_uid, f"ID:{master_uid}")

    master_items = [w for w in waiting if str(w["user_id"]) == master_uid]

    if not master_items:
        await query.edit_message_text(f"У майстра {master_name} немає очікувань.")
        return

    await query.edit_message_text(f"Очікування майстра {master_name}:")

    for item in master_items:
        order_info = item.get("order_info", "Інформація відсутня")
        date_str = item.get("date", "")
        link = item.get("msg_link", "")
        text = "Очікує запчастина\nДата: " + date_str + "\n\n" + order_info + "\nПосилання: " + link
        await update.effective_message.reply_text(text)


async def cancel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("❌ Скасовано.", reply_markup=get_main_menu(str(update.effective_user.id)))
    return ConversationHandler.END
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(no_stock, pattern="^no_stock_[0-9]+$"))
    app.add_handler(CallbackQueryHandler(received_part, pattern="^received_"))
    app.add_handler(MessageHandler(filters.TEXT & filters.Regex("Очікую запчастин"), waiting_parts_menu))
    app.add_handler(MessageHandler(filters.TEXT & filters.Regex("Очікування запчастин майстрів"), admin_waiting_list))
    app.add_handler(CallbackQueryHandler(admin_master_parts, pattern="^admin_master_"))
    
    # Add /report command for team lead
