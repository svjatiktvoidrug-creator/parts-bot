...
(HISTORY_PHOTO, HISTORY_TEXT) = range(9, 11)
...
async def history_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("✅ *Запит відправлено!*", parse_mode="Markdown", reply_markup=get_main_menu())
    except Exception as e:
...
async def order_done(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await context.bot.send_message(
            chat_id=user_id,
            text="✅ *Ваше замовлення опрацьовано!*

Менеджер підтвердив виконання замовлення.",
            parse_mode="Markdown"
        )
    except Exception as e:
...
def main():
    ...
    history_conv = ConversationHandler(
        entry_points=[
            MessageHandler(filters.TEXT & filters.Regex("Історія заміни запчастин"), history_start),
        ],
        states={
